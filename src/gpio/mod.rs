/**
 * We are using a memory mapped implementation to increase gpio switching frequency
 * 
 * https://kilobaser.com/beaglebone-black-gpios/ 
 * The AM335x has four built-in GPIO controllers, named gpio0[], gpio1[], gpio2[] and gpio3[]. 
 * For each controller, there is one page of memory which controls each gpio controller.
 * Each controller is responsible for 32 GPIOs. Each 32bit word has a specific function. 
 * Like pin configuration, controlling or setting a specific pin-state. 
 * Each bit in each of these words controls a GPIO pin. 
 * Choose function by choosing the word, choose GPIO by choosing the bit. 
 * 
 * https://kilobaser.com/wp-content/uploads/2021/02/BBB_SRM.pdf 
 * Table 12 and 13 were used to determine the P[8/9]_pin_number on expansion header -> gpio controller value in chip
 */
...
impl Gpio {
    pub fn open(index: usize) -> Arc<Gpio> {
        ...
        //     panic!("Cannot acquire GPIO at {index}. Did you call Gpio::open twice?");
        // }

        let oe = Mutex::new(unsafe {
        ...
        let datain = unsafe {
            base.offset(GPIO_DATAIN_REGISTER) as *mut u32
        };
    ...
    pub fn get_pin(self: &Arc<Self>, index: usize) -> Pin {
        return Pin { gpio: self.clone(), index };
    }
}
...
//   file.write(gpio.as_bytes()).expect("Write failed");
...
// pub fn read_gpio_value(gpio: &str) -> u8 {
//   let filepath = format!("/sys/class/gpio/gpio{}/value", gpio);
//   let value = fs::read_to_string(&filepath).expect("Could not read from file");

//   // Parse the value as u8 (0 or 1) and return it.
//   value.trim().parse::<u8>().expect("Failed to convert value")
// }
